...
class CaptureTexture
{
  ...
  public:
    ...
    void setCaptureTexture(CaptureTexture* manager) { m_manager = manager; }
  private:
    bool m_isSleeping;
    CaptureTexture* m_manager;
  };
...
public:
  ...
  static HRESULT createInst(CaptureTexture** ppEngine)
  {
    HRESULT hr = S_OK;
    *ppEngine = nullptr;

    CaptureTexture* pEngine = new (std::nothrow)CaptureTexture();
    if (pEngine != nullptr)
    {
      *ppEngine = pEngine;
      pEngine = nullptr;
  ...
  HRESULT initCaptureTexture(IUnknown* pUnk);
