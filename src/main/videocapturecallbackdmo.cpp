...
VideoCaptureCBDMO::VideoCaptureCBDMO()
  : m_ref(1)
...
STDMETHODIMP VideoCaptureCBDMO::QueryInterface(REFIID riid, void** ppv)
{
  static const QITAB qit[] =
  {
    QITABENT(VideoCaptureCBDMO, IMFSourceReaderCallback)
    , { 0 }
  };
  return QISearch(this, qit, riid, ppv);
}

STDMETHODIMP_(ULONG) VideoCaptureCBDMO::AddRef()
{
  return InterlockedIncrement(&m_ref);
}

STDMETHODIMP_(ULONG) VideoCaptureCBDMO::Release()
{
...
HRESULT VideoCaptureCBDMO::setSourceReader(IMFSourceReader* sourceReader)
{
  HRESULT hr = E_FAIL;
...
HRESULT VideoCaptureCBDMO::setDx11Renerer(DX11BaseRenderer* renderer)
{
  if (!renderer)
  {
    return E_FAIL;
...
STDMETHODIMP VideoCaptureCBDMO::OnReadSample(
    HRESULT hrStatus
    , DWORD dwStreamIndex
    , DWORD dwStreamFlags
...
UINT32 VideoCaptureCBDMO::getOptimizedFormatIndex()
{
