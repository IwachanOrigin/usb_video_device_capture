...
HRESULT VideoCaptureCB::setSourceReader(IMFSourceReader* sourceReader)
{
  ...
  hr = CoCreateInstance(CLSID_VideoProcessorMFT, nullptr, CLSCTX_INPROC_SERVER, IID_IUnknown, (void**)colorConvTransformUnk.GetAddressOf());
  if (FAILED(hr))
  {
  ...
  if (SUCCEEDED(hrStatus))
  {
    if (sample)
    {
      ...
      if (FAILED(hr))
      {
        ...
        {
          std::cerr << "ProcessInput is MF_E_NOTACCEPTING" << std::endl;
        }
